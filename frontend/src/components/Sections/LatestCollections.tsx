...
function LatestCollections({latestCollections, mintActivities}: LatestCollectionsProps) {
  ...
  useEffect(() => {
    if (latestCollections.length === 0) return
  ...
  // Si pas de collections, on affiche un message de chargement
        ...
        <div className='lg:col-span-2'>
          <div className='bg-dark-800 text-white rounded-lg overflow-hidden shadow-md hover:shadow-lg transition duration-300'>
            <div className='relative h-[300px] sm:h-[400px] lg:h-[500px]'>
              <img 
                src={getAssetUrl(currentCollection?.image || '')} 
              ...
              <div className='absolute inset-0 bg-gradient-to-t from-dark-900 to-transparent flex items-end justify-center'>
                <div className='text-center p-4 sm:p-6 w-full'>
                  <h3 className='text-xl sm:text-2xl lg:text-3xl font-cabin font-bold mb-2 text-primary-300'>
                    {currentCollection?.name || ''}
                  </h3>
                  <p className='text-sm sm:text-base text-gray-300 mb-2 font-quicksand line-clamp-2 sm:line-clamp-none'>
                    {currentCollection?.description || ''}
                  </p>
                  <p className='text-xs sm:text-sm text-primary-300'>
                    by {currentCollection?.artist || ''}
                  </p>
                </div>
              </div>
            </div>
