...
export const AlertProvider = ({ children }: { children: ReactNode }) => {
  ...
  const showAlert = useCallback((message: string, type: AlertType = 'info', duration: number = 5000) => {
    // Check if we already have this notification to prevent duplicates
    const existingAlert = alerts.find(n => n.message === message && n.type === type)
    if (existingAlert) {
...
const AlertComponent: React.FC<AlertProps> = ({ id, message, type, duration, onClose }) => {
  ...
  return (
    <div 
      className={`mb-2 p-3 rounded-lg shadow-lg text-sm font-medium animate-fadeIn relative overflow-hidden ${getAlertStyles()}`}
    >
...
// Default export for the Alert component
