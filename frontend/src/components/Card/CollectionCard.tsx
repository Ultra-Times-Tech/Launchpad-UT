...
function CollectionCard({id, name, description, image, artist}: CollectionCardProps) {
  return (
    <div className='h-[450px] bg-dark-800 rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition duration-300 flex flex-col'>
      <div className='relative h-48 flex-shrink-0'>
        <img src={getAssetUrl(image)} alt={name} className='w-full h-full object-cover' />
      </div>

      <div className='p-4 flex flex-col h-[258px]'>
        ...
        <Link to={`/collection/${id}`} className='mt-auto block w-full'>
          <button className='w-full bg-primary-500 hover:bg-primary-600 text-white font-bold py-2 px-4 rounded-lg transition duration-200'>VIEW COLLECTION</button>
        </Link>
      </div>
    </div>
  )
