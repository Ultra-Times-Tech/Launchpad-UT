...
function CollectionForm({collection, onSave, onCancel, title, submitLabel}: CollectionFormProps) {
  ...
  return (
    <div className='fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 overflow-y-auto' onClick={handleBackdropClick} data-aos='fade'>
      <div ref={modalRef} className='bg-dark-900 rounded-xl w-full max-w-lg mx-4 my-8 overflow-hidden border border-dark-600 shadow-2xl max-h-[90vh]' data-aos='zoom-in'>
        <div className='bg-dark-900'>
          <div className='flex justify-between items-center p-6 border-b border-dark-600 sticky top-0 z-10 bg-dark-900'>
          ...
          <div className='p-6 overflow-y-auto max-h-[calc(90vh-80px)]'>
            <form onSubmit={handleSubmit} className='space-y-6'>
              <div data-aos='fade-up' data-aos-delay='100'>
                <label className='block text-gray-200 mb-2 font-medium'>{t('name')}</label>
                <input type='text' value={name} onChange={e => setName(e.target.value)} className='w-full bg-dark-800 text-white rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-primary-500 border border-dark-600 transition-all duration-200' required placeholder={t('collection_name_placeholder')} />
              </div>

              <div data-aos='fade-up' data-aos-delay='200'>
                <label className='block text-gray-200 mb-2 font-medium'>{t('description')}</label>
                <textarea value={content} onChange={e => setContent(e.target.value)} className='w-full bg-dark-800 text-white rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-primary-500 min-h-[100px] border border-dark-600 transition-all duration-200' placeholder={t('collection_description_placeholder')} />
              </div>

              <div data-aos='fade-up' data-aos-delay='300'>
                <label className='block text-gray-200 mb-2 font-medium'>{t('state')}</label>
                <div className='relative'>
                  <select value={state} onChange={e => setState(Number(e.target.value))} className='w-full bg-dark-800 text-white rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-primary-500 border border-dark-600 transition-all duration-200 appearance-none pr-10'>
              ...
              <div className='grid grid-cols-1 md:grid-cols-2 gap-4' data-aos='fade-up' data-aos-delay='400'>
                <div>
                  <label className='block text-gray-200 mb-2 font-medium'>{t('publish_date')}</label>
                  <input type='datetime-local' value={publishUp ? publishUp.replace(' ', 'T') : ''} onChange={e => setPublishUp(e.target.value.replace('T', ' '))} className='w-full bg-dark-800 text-white rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-primary-500 border border-dark-600 transition-all duration-200 [color-scheme:dark]' />
                </div>

                <div>
                  <label className='block text-gray-200 mb-2 font-medium'>{t('publish_end_date')}</label>
                  <input type='datetime-local' value={publishDown ? publishDown.replace(' ', 'T') : ''} onChange={e => setPublishDown(e.target.value.replace('T', ' '))} className='w-full bg-dark-800 text-white rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-primary-500 border border-dark-600 transition-all duration-200 [color-scheme:dark]' />
                </div>
              </div>
              ...
              <div data-aos='fade-up' data-aos-delay='500'>
                <div className='flex items-center justify-between mb-2'>
                  ...
                  <div className='bg-primary-900/50 px-3 py-1 rounded-md text-white text-sm border border-primary-500/30'>{t('dimensions', {width: OPTIMAL_WIDTH, height: OPTIMAL_HEIGHT})}</div>
                </div>

                <div className='flex flex-col items-center justify-center border-2 border-dashed border-dark-600 rounded-lg p-6 bg-dark-800 transition-all duration-300 hover:border-primary-500/50 mb-3'>
                  ...
                  {uploading ? (
                    ...
                    <button type='button' onClick={() => fileInputRef.current?.click()} className='flex flex-col items-center justify-center py-4 w-full' aria-label={t('click_to_upload')}>
                      <svg xmlns='http://www.w3.org/2000/svg' className='h-12 w-12 text-primary-500 mb-3' fill='none' viewBox='0 0 24 24' stroke='currentColor'>
                      ...
                      <div className='bg-primary-500/20 px-3 py-1 rounded-md'>
                        <p className='text-white text-sm'>{t('min_dimensions', {width: OPTIMAL_WIDTH, height: OPTIMAL_HEIGHT})}</p>
                      </div>
                    </button>
                  )}
                </div>
                ...
                {imageError && (
                  <div className='bg-red-900/30 border border-red-500/30 text-white text-sm p-3 rounded-md mt-2 mb-3' data-aos='fade-up'>
                ...
                {imageUrl && !imageError && (
                  <div className='mt-3 p-3 border border-dark-600 rounded-lg bg-dark-800' data-aos='fade-up'>
              ...
              <div className='flex justify-end space-x-3 pt-4' data-aos='fade-up' data-aos-delay='600'>
                <button type='button' onClick={onCancel} className='px-5 py-2.5 bg-dark-700 text-white rounded-lg hover:bg-dark-600 transition-all duration-200 font-medium'>
                  {t('cancel')}
                </button>
                <button type='submit' className='px-5 py-2.5 bg-primary-600 text-white rounded-lg hover:bg-primary-500 transition-all duration-200 font-medium disabled:opacity-50 disabled:cursor-not-allowed' disabled={uploading || !!imageError}>
                  {submitLabel}
                </button>
              </div>
            </form>
