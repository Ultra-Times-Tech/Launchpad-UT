      ...
      <div ref={modalRef} className="bg-dark-800 rounded-lg w-full max-w-md mx-4 overflow-hidden border border-dark-600 shadow-xl">
        <div className="p-6">
          ...
          <div className="my-6">
            ...
            <p className="text-gray-400 text-center mb-4 text-sm"><strong>{collection.name}</strong></p>
          ...
          <form onSubmit={handleSubmit}>
              ...
              <div className="flex flex-col items-center justify-center border-2 border-dashed border-dark-600 rounded-lg p-4 bg-dark-700 transition-colors hover:border-primary-500/50 mb-3">
                ...
                {uploading ? (
                  ...
                  <button
                    ...
                    aria-label="Cliquez pour télécharger une image"
                  >
                    ...
                    <div className="bg-primary-500/20 px-3 py-1 rounded-md">
                      <p className="text-white text-sm">Taille minimale: {OPTIMAL_WIDTH}x{OPTIMAL_HEIGHT}px</p>
                    </div>
                  </button>
                )}
              </div>
            ...
            <div className="flex justify-end space-x-3 pt-2">
              <button 
                type="button" 
                onClick={onCancel}
                className="px-5 py-2 bg-dark-600 text-white rounded-lg hover:bg-dark-500 transition-colors"
              >
                Annuler
              </button>
              <button 
                type="submit"
                className="px-5 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-500 transition-colors font-medium"
                disabled={uploading || !!imageError}
              >
                {submitLabel}
              </button>
            </div>
...
function MyCollectionsPage() {
  ...
  const handleSaveCollection = async (updatedCollection: Partial<Collection>) => {
    try {
      ...
      setCollections(prev => 
        prev.map(c => c.id === updatedCollection.id ? {...c, ...updatedCollection} as Collection : c)
      )
    ...
    <div className='container mx-auto px-4 py-8'>
      <div className='flex justify-between items-center mb-8'>
        ...
        <div className='flex items-center space-x-4'>
          ...
          <button 
            className='bg-primary-600 hover:bg-primary-500 text-white rounded-lg px-4 py-2 flex items-center transition-colors'
      ...
      {loading ? (
        ...
        <div className='text-center py-12'>
          ...
          {!searchTerm && (
            <button
              onClick={handleCreateClick}
              className='mt-4 px-6 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-500 transition-colors'
            >
